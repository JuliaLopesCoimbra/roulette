...
export default function BuyAdCPC() {
  ...
  const [goal, setGoal] = useState("click"); // CPC = click
          ...
          <AnimatePresence mode="wait">
            {step === 1 && (
              <Step1Intro
                key="step1"
                next={() => setStep(2)}
                setGoal={setGoal} // mantém a mesma API do CPV
                // se quiser travar em "click", comente a prop acima e remova a opção "view" no Step1Intro
              />
            )}
            {step === 2 && (
            ...
            {step === 3 && (
              <Step3Segmentation key="step3" next={next} back={back} />
            )}
